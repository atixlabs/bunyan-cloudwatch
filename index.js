...
function CloudWatchStream ( opts ) {
  ...
  this.prettyStream = new PrettyStream ( { useColor: false } )
}
...
CloudWatchStream.prototype._writeLogs = function _writeLogs () {
  ...
  var log = {
    ...
    logEvents: this.queuedLogs.map ( createCWLog.bind ( this ) )
  };
  this.queuedLogs = [];
  var obj = this;
  writeLog ();
...
function createCWLog ( bunyanLog ) {
  ...
  if (typeof bunyanLog === 'string') {
    data = JSON.parse ( bunyanLog )

  }
  var formatted = this.prettyStream.formatRecord ( data );
